...
%     ); 
    ...
    struct(...
        'name', '多道字典反演（K=10）', ...
        'flag', 'DLSR', ...
        'regParam', struct('lambda', 0.5, 'gamma', 0.4), ...
        'parampkgs', bsSetFields(GSparseInvParam, {'sparsity', 10; 'nMultipleTrace', 10}), ...
        'options', bsSetFields(GInvParam.seisInvOptions, ...
            {'maxIter', 5; 'innerIter', 10; 'initRegParam', 0.5}), ...
        'showFiltCoef', 0.8, ...
        'isSaveSegy', 1, ...
        'load', struct(...
            'mode', 'off', ...
...
if(methods{end}.parampkgs.nMultipleTrace == 1)
    methods{end}.name = '单道字典反演';
end

%% 得到初步的反演结果
fprintf('反演剖面中...\n');
invResults = bsPostInvTrueMultiTracesPlusLateralContinuty(GInvParam, inIds, crossIds, timeLine, methods);
% methods{end}.name = '多道字典反演';
% methods{end}.load.mode = 'off';
% 
% invResults(length(invResults)+1) = bsPostInvTrueMultiTracesPlusLateralContinuty(GInvParam, inIds, crossIds, timeLine, methods(end));
...
for k = 1 : nK
    ...
    if k == 1
        str = sprintf('%s-单道高分辨率', methods{end}.name);
    else
        str = sprintf('%s-多道高分辨率 (K=%d)', methods{end}.name, Ks(k));
    end
    
    finalResults{nRes + spos} = bsPostRebuildByCSRWithWholeProcess(GInvParam, timeLine, wellLogs, ...
        methods(nRes), invResults{nRes}, str, ...
        'gamma', csrGamma, ...
        'ratio_to_reconstruction', 1, ...
        'trainNum', trainNum, ...
...
finalResults{1}.showFiltCoef = 1;
finalResults{2}.showFiltCoef = 1;
...
for k = 1 : ceil(nFigure/nsub_figure)
    ...
    set(gcf, 'position', [141          98        1623         753]);
end
